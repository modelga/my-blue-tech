...
export default async function TimelinePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [timeline, entries] = await Promise.all([getTimeline(id), getTimelineEntries(id)]);
  ...
  return (
    <div>
      <div style={formPageHeader}>
        ...
        {timeline.description && <p style={{ margin: "0.25rem 0 0", color: "#6b7280", fontSize: "0.95rem" }}>{timeline.description}</p>}
      </div>
