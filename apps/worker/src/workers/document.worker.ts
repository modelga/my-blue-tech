...
export function startDocumentWorker(boss: PgBoss, pool: Pool) {
  ...
  boss.work<InitializeDocumentJob>("initialize-document", async (job) => {
    ...
    if (result.capabilityFailure) {
      throw new Error(
        `[initialize-document] capability failure for ${documentId}: ${result.failureReason}`,
      );
    }
